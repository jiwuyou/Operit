...
本文档描述一个**独立于工作流系统**的外部交互接口：外部应用通过发送广播 Intent（`com.wuxianc.operit.EXTERNAL_CHAT`）向 Operit 发起一次“发送消息给 AI”的请求，并通过另一个广播接收执行结果。
...
- **请求 Action**：`com.wuxianc.operit.EXTERNAL_CHAT`
- **默认回传 Action**：`com.wuxianc.operit.EXTERNAL_CHAT_RESULT`
...
| `reply_action` | `String` | 否 | `com.wuxianc.operit.EXTERNAL_CHAT_RESULT` | 指定回传广播 action |
...
adb shell am broadcast \
  -a com.wuxianc.operit.EXTERNAL_CHAT \
...
- 默认 action：`com.wuxianc.operit.EXTERNAL_CHAT_RESULT`
...
class ExternalChatResultReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        if (intent.action != "com.wuxianc.operit.EXTERNAL_CHAT_RESULT") return
...
<receiver
    ...
    <intent-filter>
        <action android:name="com.wuxianc.operit.EXTERNAL_CHAT_RESULT" />
    </intent-filter>
</receiver>
```
